...
function getAggregates() {
    fs.readFile('./data/aggregate.json', 'utf-8', function(err,data){
        ...
        let count = 0;
        for(let i = 0; i< 20; i++) {
            //topSet.push('BTC-'+topCoins[i].symbol);
            if(marketArray.indexOf('BTC-'+topCoins[i].symbol) != -1 && 
                topCoins[i].percent_change_1h > 0.5 && count <= 10 && 
                (!purchaseObj['BTC-'+topCoins[i].symbol] || purchaseObj['BTC-'+topCoins[i].symbol].totalHoldings == 0)){
                count ++;
                console.log('BTC-'+topCoins[i].symbol,'    ',topCoins[i].percent_change_1h,'    ',topCoins[i].percent_change_24h,'   ',topCoins[i].price_btc);
                let buyObj = {};
                ...
                if(!purchaseObj['BTC-'+topCoins[i].symbol]) {
                    purchaseObj['BTC-'+topCoins[i].symbol] = {buys: [], sells: []};
                }
    ...
    fs.readFile('./data/markets.json', 'utf-8', function(err,data){
        ...
        marketList.forEach(function (market) {
            if(market.MarketName.indexOf('BTC-') == 0) {
                ...
                // if(topSet.indexOf(market.MarketName) != -1){
                //     console.log(market.MarketName);
                // }
            }
        });
        getAggregates();
        setTimeout(function(){getAggregates();}, 2000);
    });
}
